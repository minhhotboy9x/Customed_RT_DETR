...
__all__ = ['CustomedPostProcessor', 'CustomedPostProcessor2', 'CustomedPostProcessor3', 'CustomedPostProcessor4']
...
@register
class CustomedPostProcessor4(nn.Module):
    '''
    box output in format 
    '''
    __share__ = ['num_classes', 'use_focal_loss', 'num_top_queries', 'remap_mscoco_category', 'conf']
    ...
    def forward(self, outputs, orig_target_sizes):
        ...
        logits, boxes, topk_ind = outputs['pred_logits'], outputs['pred_boxes'], outputs['topk_ind']
        # orig_target_sizes = torch.stack([t["orig_size"] for t in targets], dim=0)        
        ...
        if self.use_focal_loss:
            ...
            index = index // self.num_classes
            boxes = bbox_pred.gather(dim=1, index=index.unsqueeze(-1).repeat(1, 1, bbox_pred.shape[-1]))
            topk_anchor_idx = topk_ind.gather(dim=1, index=index)
        ...
        results = []
        for lab, box, sco, anchor in zip(labels, boxes, scores, topk_anchor_idx):
            result = dict(labels=lab, boxes=box, scores=sco, anchor_idx=anchor)
            results.append(result)
