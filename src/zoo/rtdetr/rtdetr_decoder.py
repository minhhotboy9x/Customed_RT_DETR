...
class RTDETRTransformer(nn.Module):
    ...
    def _generate_anchors(self,
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            ...
            # grid_y shape of [h, w] contain column id
            # grid_x shape of [h, w] contain row id
            grid_xy = torch.stack([grid_x, grid_y], -1) # [h, w, 2]
            valid_WH = torch.tensor([w, h]).to(dtype) # [2]
            grid_xy = (grid_xy.unsqueeze(0) + 0.5) / valid_WH # [1, h, w, 2], rel coords of center each grid
            wh = torch.ones_like(grid_xy) * grid_size * (2.0 ** lvl) # [1, h, w, 2] 
            anchors.append(torch.concat([grid_xy, wh], -1).reshape(-1, h * w, 4)) # [1, h*w, 4]

        anchors = torch.concat(anchors, 1).to(device) # [1, sum(w[i]*h[i]) i in range(len(spatial_shapes)), 4]
